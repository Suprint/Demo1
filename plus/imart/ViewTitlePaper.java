 ...
 * also scrolled.
 */
public class ViewTitlePaper extends View implements PageIndicator {
    ...
    public ViewTitlePaper(Context context) {
        this(context, null);
    }

    public ViewTitlePaper(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.vpiTitlePageIndicatorStyle);
    }

    public ViewTitlePaper(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if ((bound.left > left && bound.left < right) || (bound.right > left && bound.right < right)) {
                ...
                canvas.drawText(pageTitle, 0, pageTitle.length(), bound.left, bound.bottom, mPaintText);
                ...
                if (currentPage && currentSelected) {
        			...
        			backPaint.setARGB(100, 100, 50, 50);
        			backPaint.setAntiAlias(true);
        			backPaint.setStyle(Paint.Style.FILL_AND_STROKE);
        			
                    Rect underlineBounds = bounds.get(page);
                   	RectF oval = new RectF((underlineBounds.left  - mFooterIndicatorUnderlinePadding), ((bound.top) - ((bound.top)/3.0f)), (underlineBounds.right + mFooterIndicatorUnderlinePadding), ((bound.bottom) + ((bound.bottom)/3.0f)));

                   	canvas.drawRoundRect(oval, 10, 10, backPaint);
                    canvas.drawText(pageTitle, 0, pageTitle.length(), bound.left, bound.bottom, mPaintText);
                }
            }
        }
